...
from ..models.wan_video_motioncanvas import WanModel
...
from ..models.wan_video_motioncanvas import RMSNorm, sinusoidal_embedding_1d
from ..models.wan_video_vae import RMS_norm, CausalConv3d, Upsample
from ..models.wan_video_motion_controller import WanMotionControllerModel
from .tracker_utils import get_video_track_video


class ZeroConv3D(torch.nn.Module):
    def __init__(self, channels: int):
        super().__init__()
        self.conv = torch.nn.Conv3d(channels, channels, kernel_size=1)
...
class WanVideoPipeline_motioncanvas(BasePipeline):
    def __init__(self, device="cuda", torch_dtype=torch.float16, tokenizer_path=None):
        ...
        self.bbox_zeroconv = ZeroConv3D(16)
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        self.dit = model_manager.fetch_model("wan_video_motioncanvas")
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False):
        ...
        pipe = WanVideoPipeline_motioncanvas(device=device, torch_dtype=torch_dtype)
    ...
    def __call__(
        ...
        if training_visualization and latents_shape is not None:
            num_frames, height, width = latents_shape[2], latents_shape[3], latents_shape[4]
        else:
        ...
        # init noise
        if training_visualization and latents_shape is not None:
            noise = self.generate_noise(latents_shape, seed=seed, device=rand_device, dtype=torch.float32)
        else:
            noise = self.generate_noise((1, 16, (num_frames - 1) // 4 + 1, height//8, width//8), 
        ...
        if control_video is not None:
            self.load_models_to_device(["image_encoder", "vae"])
            image_emb = self.prepare_controlnet_kwargs(control_video, (num_frames-1)*4+1, height, width, **image_emb, **tiler_kwargs)
        ...
        for vid_i in range(len(frames)):
            _frames = self.tensor2video(frames[vid_i])
            frames_list.append(_frames)
    ...
    **kwargs,
    ...
    if dit.has_image_input:
        ...
        if traj_video is not None:
            traj_video = torch.cat([traj_video, y], dim=1)
    
    B, F_total = x.shape[0], x.shape[2] 
    F = x.shape[2]
    time_indices = torch.zeros((B, F_total), dtype=torch.long)
    time_indices[:, :F] = torch.arange(F).unsqueeze(0).expand(B, -1)
    ...
    original_F = f
    if traj_video is not None:
        traj_video, (original_F, _, _) = dit.patchify(traj_video)
        ...
        x = torch.cat([x, traj_video], dim=1)
