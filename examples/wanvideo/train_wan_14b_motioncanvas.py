...
class LightningModelForTrain(pl.LightningModule):
    def __init__(
        ...
        if image_encoder_path is not None:
            ...
            model_manager.load_models([image_encoder_path], torch_dtype=torch.float32) # Image Encoder is loaded with float32
        else:
        ...
        if os.path.isfile(dit_path):
            model_manager.load_models([dit_path], torch_dtype=torch.bfloat16)
        else:
            dit_path = dit_path.split(",")
            model_manager.load_models([dit_path], torch_dtype=torch.bfloat16)
        
        self.pipe = WanVideoPipeline_motioncanvas.from_model_manager(model_manager, torch_dtype=torch.bfloat16)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.visualize_training and (self.global_step % self.visualize_step == 0):
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.02, total_iters=2000)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Whether to visualize during training.",
    )
...
def train(args, logger):
    ...
    resume_checkpoint_path = os.path.join(args.output_path, "resume_checkpoints", "last.ckpt")
